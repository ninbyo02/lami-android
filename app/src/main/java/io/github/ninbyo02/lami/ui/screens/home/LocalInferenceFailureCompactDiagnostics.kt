...
internal data class LocalInferenceFailureCompactInput(
    ...
    val edgeGalleryE2bAllowlistMaxTokens: String = "unavailable",
    val gpuMaxTokensAlignment: String = "unavailable",
...
internal fun buildLocalInferenceFailureCompactDiagnosticsText(
    ...
    return (
        ...
        "edge_gallery_e2b_allowlist_max_tokens=${input.edgeGalleryE2bAllowlistMaxTokens}",
        "gpu_max_tokens_alignment=${input.gpuMaxTokensAlignment}",
...
private fun buildStandardGpuRuntimeAlignmentCandidateCompactDiagnosticLines(
    ...
    STANDARD_GPU_RUNTIME_ALIGNMENT_CANDIDATE_DIAGNOSTIC_KEYS.map { key ->
...
private fun buildStandardGpuMinimalRuntimeCandidateCompactDiagnosticLines(
    diagnostics: Map<String, String>,
): List<String> {
    if (diagnostics.keys.none { it in STANDARD_GPU_MINIMAL_RUNTIME_CANDIDATE_DIAGNOSTIC_KEYS }) return emptyList()
    return STANDARD_GPU_MINIMAL_RUNTIME_CANDIDATE_DIAGNOSTIC_KEYS.map { key ->
        "$key=${diagnostics[key]?.let(::escapeLocalInferenceFailureValue) ?: "unavailable"}"
    }
}

private fun extractStandardGpuMinimalRuntimeCandidateDiagnostics(text: String?): Map<String, String> =
    parseLocalInferenceFailureDiagnosticsText(text)
        .filterKeys { it in STANDARD_GPU_MINIMAL_RUNTIME_CANDIDATE_DIAGNOSTIC_KEYS }
...
private val GPU_OUTPUT_QUALITY_DIAGNOSTIC_KEYS = listOf(
    ...
    "cpu_compare_engine_initialize_finished",
    "cpu_compare_conversation_create_finished",
    "cpu_compare_generate_started",
    "cpu_compare_callback_invoked_count",
    "cpu_compare_empty_text_count",
    "cpu_compare_non_empty_text_count",
    "cpu_compare_first_non_empty_text_elapsed_ms",
    "cpu_compare_done_true_seen",
    "cpu_compare_exception_class",
    "cpu_compare_exception_message",
    "cpu_callback_invoked_count",
    "cpu_callback_empty_text_count",
    "cpu_callback_non_empty_text_count",
    "cpu_avg_chunk_length",
    "cpu_median_chunk_length",
    "cpu_p90_chunk_length",
    "cpu_p95_chunk_length",
    "cpu_one_char_chunk_count",
    "cpu_two_char_or_less_chunk_count",
    "cpu_one_char_chunk_ratio",
    "gpu_avg_chunk_length",
    "cpu_callback_count",
    "gpu_callback_count",
    "cpu_two_char_or_less_ratio",
    "cpu_chunk_length_histogram",
    ...
    "cpu_output_suspicious_fragment_reason",
    ...
    "cpu_gpu_avg_chunk_length_ratio",
    "cpu_gpu_two_char_or_less_ratio_delta",
    "cpu_gpu_callback_count_delta",
    "cpu_gpu_raw_text_similarity_hint",
    "cpu_gpu_same_prompt",
    "cpu_gpu_same_max_tokens",
    "cpu_gpu_same_sampler_config_hint",
...
internal fun buildLocalInferenceFailureCompactInputFromTrace(
    ...
    return buildLocalInferenceFailureCompactInput(
        LocalInferenceFailureCompactInputContext(
            ...
            before = before,
            after = after,
            resolvedFailureExceptionClass = resolvedFailureExceptionClass,
            resolvedFailureExceptionMessage = resolvedFailureExceptionMessage,
            exceptionExpansion = exceptionExpansion,
            previousInvocationStillProcessing = previousInvocationStillProcessing,
            gpuEngineCreateTimeoutSuspected = gpuEngineCreateTimeoutSuspected,
            gpuFailureClassification = gpuFailureClassification,
            effectiveLiteRtLmErrorClassification = effectiveLiteRtLmErrorClassification,
            galleryStackProbe = galleryStackProbe,
            runtimeAlignmentProbe = runtimeAlignmentProbe,
        ),
    )
}

private data class LocalInferenceFailureCompactInputContext(
    val inputPrompt: String,
    val preferredBackendSetting: PreferredBackendDryRunSetting,
    ...
    val routeContext: LocalRouteDiagnosticContext?,
    val timeout: Boolean,
    val modelName: String?,
    val modelFile: String?,
    val ttsRequested: Boolean,
    val dbRequested: Boolean,
    val markdownRequested: Boolean,
    val streamingRequested: Boolean,
    val processPid: String,
    val parsed: Map<String, String>,
    val probeDiagnostics: Map<String, String>,
    val cpuRouteDiagnostics: Map<String, String>,
    val before: MemorySnapshot?,
    val after: MemorySnapshot?,
    val resolvedFailureExceptionClass: String,
    val resolvedFailureExceptionMessage: String,
    val exceptionExpansion: LocalFailureExceptionExpansion,
    val previousInvocationStillProcessing: Boolean,
    val gpuEngineCreateTimeoutSuspected: String,
    val gpuFailureClassification: GpuLiteRtFailureClassification,
    val effectiveLiteRtLmErrorClassification: LiteRtLmErrorClassification,
    val galleryStackProbe: GalleryStackGpuProbeRuntimeDiagnostics,
    val runtimeAlignmentProbe: RuntimeAlignmentProbeDiagnostics,
)

private fun buildLocalInferenceFailureCompactInput(
    context: LocalInferenceFailureCompactInputContext,
): LocalInferenceFailureCompactInput = with(context) {
    LocalInferenceFailureCompactInput(
        inputPrompt = inputPrompt,
        preferredBackendSetting = preferredBackendSetting,
        npuStandardRouteMode = npuStandardRouteMode,
        ...
        edgeGalleryE2bAllowlistMaxTokens = parsed["edge_gallery_e2b_allowlist_max_tokens"] ?: "unavailable",
        gpuMaxTokensAlignment = parsed["gpu_max_tokens_alignment"] ?: "unavailable",
