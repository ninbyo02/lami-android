...
class SpriteSettingsDropdownTestUtil(
    private val composeTestRule: AndroidComposeTestRule<*, *>,
    private val ensureAnimTabSelected: () -> Unit,
    private val waitForNodeWithTag: (String, Long) -> Unit,
    private val scrollToAnimationDropdownAnchor: (String) -> Unit,
) {
    private var lastAnchorCandidateDiagnostics: String = ""
    private var lastLabelNodeCountDiagnostics: String = ""

    fun selectAnimationTypeByAnchor(
        label: String,
        ...
        repeat(maxAttempts) { attempt ->
            ...
            waitForNodeWithTag("spriteBaseIntervalInput", timeoutMillis)
            ...
            composeTestRule.waitForIdle()
            val labelClicked = clickLabelNode(label, timeoutMillis)
            if (!labelClicked) {
            ...
            val selectionConfirmed = runCatching {
                waitForSelectionSuccess(label, anchorTag, timeoutMillis)
            }.getOrElse { error ->
                lastError = error
                false
            }
        ...
        val diagnostics = buildFailureDiagnostics(label, anchorTag)
    ...
    private fun collectAnchorCandidates(anchorTag: String): List<AnchorCandidate> {
    ...
    private fun clickLabelNode(label: String, timeoutMillis: Long): Boolean {
        runCatching {
            composeTestRule.waitUntil(timeoutMillis = timeoutMillis) {
                labelNodeCount(label) > 0
            }
        }
        val unmergedCollection = composeTestRule.onAllNodesWithText(label, useUnmergedTree = true)
        val unmergedNodes = runCatching { unmergedCollection.fetchSemanticsNodes() }.getOrDefault(emptyList())
        val mergedCollection = composeTestRule.onAllNodesWithText(label)
        val mergedNodes = runCatching { mergedCollection.fetchSemanticsNodes() }.getOrDefault(emptyList())
        lastLabelNodeCountDiagnostics = "labelNodes(unmerged=${unmergedNodes.size}, merged=${mergedNodes.size})"
    ...
    private fun waitForSelectionSuccess(label: String, anchorTag: String, timeoutMillis: Long): Boolean {
        try {
            composeTestRule.waitUntil(timeoutMillis = timeoutMillis) {
    ...
    private fun anchorShowsLabel(label: String, anchorTag: String): Boolean {
        val unmerged = composeTestRule.onAllNodesWithTag(anchorTag, useUnmergedTree = true)
        val unmergedNodes = runCatching { unmerged.fetchSemanticsNodes() }.getOrDefault(emptyList())
        val merged = composeTestRule.onAllNodesWithTag(anchorTag)
    ...
    private fun buildFailureDiagnostics(label: String, anchorTag: String): String {
    ...
    private fun countAnchorNodes(anchorTag: String): Int {
        ...
        val merged = runCatching {
            composeTestRule.onAllNodesWithTag(anchorTag).fetchSemanticsNodes().size
        }.getOrDefault(0)
        return maxOf(unmerged, merged)
    }

    private fun labelNodeCount(label: String): Int {
        val unmerged = runCatching {
            composeTestRule.onAllNodesWithText(label, useUnmergedTree = true).fetchSemanticsNodes().size
        }.getOrDefault(0)
        val merged = runCatching {
    ...
    private fun labelNodeCounts(label: String): String {
        val unmerged = runCatching {
            composeTestRule.onAllNodesWithText(label, useUnmergedTree = true).fetchSemanticsNodes().size
        }.getOrDefault(0)
        val merged = runCatching {
