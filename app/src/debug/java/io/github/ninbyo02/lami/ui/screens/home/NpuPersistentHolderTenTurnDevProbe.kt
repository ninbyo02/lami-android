...
internal class NpuPersistentHolderTenTurnDevProbe(
    context: Context,
) : NpuPersistentHolderTenTurnProbeRunner {
    private val appContext = context.applicationContext

    override suspend fun run(): NpuPersistentHolderTenTurnProbeState = withContext(Dispatchers.Default) {
        ...
        try {
            ...
            closeResult = api.closeHolder(
                NpuPersistentHolderCloseRequest(
                    holderId = holderId,
                    reason = "dev_ui_ten_turn_probe_close",
                ),
            )
            ...
            if (closeResult == null && createResult != null) {
                closeResult = runCatching {
                    api.closeHolder(
                        NpuPersistentHolderCloseRequest(
                            holderId = holderId,
                            reason = "dev_ui_ten_turn_probe_close_after_throwable",
                        ),
                    )
                }.getOrNull()
            ...
            NpuPersistentHolderTenTurnProbeState(
                status = "failed",
                reason = throwable.message ?: "holder_ten_turn_probe_failed",
                startedAtElapsedRealtimeMs = startedAt,
                finishedAtElapsedRealtimeMs = SystemClock.elapsedRealtime(),
                modelPathOrReason = modelPath,
                createResult = createResult,
