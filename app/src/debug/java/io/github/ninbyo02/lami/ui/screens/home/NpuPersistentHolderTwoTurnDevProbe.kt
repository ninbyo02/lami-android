...
internal class NpuPersistentHolderTwoTurnDevProbe(
    context: Context,
) : NpuPersistentHolderTwoTurnProbeRunner {
    private val appContext = context.applicationContext

    override suspend fun run(): NpuPersistentHolderTwoTurnProbeState = withContext(Dispatchers.Default) {
        ...
        try {
            ...
            if (createResult.diagnostics.holderCreateSucceeded && diagnosticsAfterCreate.holderOpen) {
                val turn1 = runTurn(
                    ...
                    holderId = holderId,
                    turnIndex = 1,
                    prompt = NPU_PERSISTENT_HOLDER_TWO_TURN_PROMPT_1,
                )
                turns += turn1
                if (turn1.decodeResult?.status == "success") {
                    turns += runTurn(
                        api = api,
                        holderId = holderId,
                        turnIndex = 2,
                        prompt = NPU_PERSISTENT_HOLDER_TWO_TURN_PROMPT_2,
                    )
                }
            }
            closeResult = api.closeHolder(
                NpuPersistentHolderCloseRequest(
                    holderId = holderId,
                    reason = "dev_ui_two_turn_probe_close",
                ),
            )
            ...
            if (closeResult == null && holderId != "unavailable") {
                closeResult = runCatching {
                    api.closeHolder(
                        NpuPersistentHolderCloseRequest(
                            holderId = holderId,
                            reason = "dev_ui_two_turn_probe_close_after_throwable",
                        ),
                    )
                }.getOrNull()
            ...
            NpuPersistentHolderTwoTurnProbeState(
                status = "failed",
                reason = throwable.message ?: "holder_two_turn_probe_failed",
                startedAtElapsedRealtimeMs = startedAt,
                finishedAtElapsedRealtimeMs = SystemClock.elapsedRealtime(),
                modelPathOrReason = modelPath,
                createResult = createResult,
