...
- **スプライト編集**: 端末内のスプライト画像を読み込み、選択範囲の編集や PNG のインポート/エクスポートが可能。長押しで連続移動ができます。
...
Thinking のデフォルトアニメーションは初期設定・破損時・リセット時に同一のJSON定義へ統一されます。
TalkShort / TalkLong / TalkCalm はそれぞれ独立した state として DataStore に保存され、保存後・画面遷移後・再起動後も個別のJSONが復元されます。
...
It includes upstream components under the **MIT License**. See `NOTICE`.

Original modifications (by ninbyo02) are licensed under the Apache License 2.0.
