...
| `Copy True Engine Holder Full Dump` | `ChatScreen.kt`, `NpuTrueEngineHolderApi.kt` | `formatNpuTrueEngineHolderCreateCloseFullDumpForCopy(...)` | Copy blocked state or native create/close details plus summary. | KEEP_PRIMARY | In `standardDebug` this does not include native result files because native execution is blocked. `trueEngineNpuProbeDebug` may include the button-triggered native create/close result, and must still confirm Session/decode/generate stayed at zero. The copy button is shown with the standalone `trueEngineNpuProbeDebug` probe entry. | `NpuPersistentHolderNativeStubTest`, `InferenceStatsSheetContentTest`, `NpuTrueEngineHolderChatScreenStandardDebugBlockedTest` | Keep until a split true Engine holder native diagnostics API replaces the runCount=0 custom JNI wrapper. |
