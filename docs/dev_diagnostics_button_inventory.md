...
| `Run Non-Streaming Repeat Test` | `ChatScreen.kt`, `NpuNonStreamingRepeatedStabilityDevProbe.kt`, `NpuNonStreamingRepeatedStabilityDiagnostics.kt` | `startNpuNonStreamingRepeatedStabilityTest()` -> `DevOnlyNpuOneTurnConversationEntry.run(...)` for 10 fixed prompts | Serial one-shot NPU decode repeat test with no pseudo streaming, no TTS, no DB, no markdown, and fallback disallowed in diagnostics. | KEEP_PRIMARY | Measures existing successful one-shot NPU/QNN/FastRPC/native cleanup repeatability before any true Engine reuse work. It does not create held Engines, sessions, conversations, or normal chat-route messages. Device result reproduced twice: 6 successes, run-7 `EngineFactory::CreateDefault`/`engine-create-failed: INTERNAL`, no fallback, no timeout, and no fresh crash. | `NpuNonStreamingRepeatedStabilityDiagnosticsTest` | Use after a one-shot NPU pass and before re-enabling true Engine create/close. Do not expand to 30/100 until staged true Engine Phase 1/2 decisions are complete. |
...
| `Run Holder Ten-Turn Probe` | `ChatScreen.kt`, `NpuPersistentHolderTenTurnDevProbe.kt` | `startNpuPersistentHolderTenTurnProbe()` -> `NativeStubNpuPersistentHolderApi` + existing one-shot NPU adapter decode | Create one DEV holder record, run exactly ten holder-gated one-shot decodes, then close once. | KEEP_PRIMARY | This validates ten holder-managed decode calls after Five-Turn passed. It is not normal chat routing and not true Engine persistent reuse. The device result matched the repeated run-7 failure shape. | `NpuPersistentHolderNativeStubTest`, `InferenceStatsSheetContentTest` | Phase 2 follow-up is the separate button-only `entrypoint_only` true Engine probe; if that physical-device artifact passes, advance to `model_assets_only`. |
...
| `Run True Engine Entrypoint Probe` | `ChatScreen.kt`, `NpuTrueEngineHolderApi.kt`, `NpuTrueEngineHolderCreateCloseDevProbe.kt` | `startNpuTrueEngineEntrypointProbe()` -> button-only `entrypoint_only` native probe in `trueEngineNpuProbeDebug` | Confirm native entrypoint reach and immediate return only. | KEEP_PRIMARY | Visible only in the true Engine probe flavor. Uses `TRUE_ENGINE_NPU_PROBE_ENTRYPOINT_ONLY_ENABLED=true` while `TRUE_ENGINE_NPU_PROBE_NATIVE_EXECUTION_ENABLED=false` remains broad-blocked. Startup UI and copy actions do not resolve model paths, class-load `Qairt244ShortMultitokenSmoke`, or load native libraries. Expected summary keeps `model_assets_create_reached=false`, `engine_settings_create_reached=false`, `engine_create_reached=false`, `session_create_count=0`, `decode_count=0`, and `generate_count=0`. | `NpuTrueEngineHolderChatScreenStandardDebugBlockedTest` | Physical-device entrypoint passed; continue to the separate button-only `model_assets_only` probe. Do not revive `true_engine_create_close_only` directly. |
| `Copy True Engine Entrypoint Summary` | `ChatScreen.kt`, `NpuTrueEngineHolderApi.kt` | `formatNpuTrueEngineEntrypointSummaryForCopy(...)` | Copy Phase 2 entrypoint-only safety keys. | KEEP_PRIMARY | Captures `selected_native_probe_mode=entrypoint_only`, entrypoint availability, startup-blocked status, broad execution disabled status, zero engine/session/decode/generate counters, and `engine_reuse_observed=unavailable`. | `NpuTrueEngineHolderChatScreenStandardDebugBlockedTest` | Keep as compact artifact for the first staged true Engine native execution check. |
| `Copy True Engine Entrypoint Full Dump` | `ChatScreen.kt`, `NpuTrueEngineHolderApi.kt` | `formatNpuTrueEngineEntrypointFullDumpForCopy(...)` | Copy native entrypoint result, native diag text, and summary. | KEEP_PRIMARY | Required physical-device artifact after pressing `Run True Engine Entrypoint Probe`; expected pass has `probe_status=completed`, `native_entrypoint_reached=true`, and no ModelAssets/EngineSettings/EngineFactory/Session/decode/generate reach. | `NpuTrueEngineHolderChatScreenStandardDebugBlockedTest` | Physical-device entrypoint passed; retain as the Phase 2 baseline artifact. |
| `Run True Engine ModelAssets Probe` | `ChatScreen.kt`, `NpuTrueEngineHolderApi.kt`, `NpuTrueEngineHolderCreateCloseDevProbe.kt` | `startNpuTrueEngineModelAssetsProbe()` -> button-only `model_assets_only` native probe in `trueEngineNpuProbeDebug` | Confirm `ModelAssets::Create` reach and return without advancing to EngineSettings or EngineFactory. | KEEP_PRIMARY | Added after physical-device `entrypoint_only` completed with `native_entrypoint_reached=true` and no deeper native work. Physical-device `model_assets_only` now reaches and returns from `ModelAssets::Create` successfully. Uses `TRUE_ENGINE_NPU_PROBE_MODEL_ASSETS_ONLY_ENABLED=true` while `TRUE_ENGINE_NPU_PROBE_NATIVE_EXECUTION_ENABLED=false` remains broad-blocked. Startup UI and copy actions do not resolve model paths, class-load `Qairt244ShortMultitokenSmoke`, or load native libraries. Expected summary keeps `engine_settings_create_reached=false`, `engine_create_reached=false`, `session_create_count=0`, `decode_count=0`, and `generate_count=0`. | `NpuTrueEngineHolderChatScreenStandardDebugBlockedTest` | If physical-device ModelAssets succeeds, add button-only `engine_settings_only`; do not revive `true_engine_create_close_only` directly. |
| `Copy True Engine ModelAssets Summary` | `ChatScreen.kt`, `NpuTrueEngineHolderApi.kt` | `formatNpuTrueEngineModelAssetsSummaryForCopy(...)` | Copy Phase 3 ModelAssets-only safety keys. | KEEP_PRIMARY | Captures `selected_native_probe_mode=model_assets_only`, ModelAssets reach/return/success, startup-blocked status, broad execution disabled status, zero engine/session/decode/generate counters, and `engine_reuse_observed=unavailable`. A native completed artifact with successful ModelAssets reach/return/success is displayed as `probe_status=completed` and `probe_reason=model_assets_only_completed`; ModelAssets failures remain visible in the summary without requiring EngineSettings or EngineFactory reach. | `NpuTrueEngineHolderChatScreenStandardDebugBlockedTest` | Keep as compact artifact for Phase 3 device review. |
| `Copy True Engine ModelAssets Full Dump` | `ChatScreen.kt`, `NpuTrueEngineHolderApi.kt` | `formatNpuTrueEngineModelAssetsFullDumpForCopy(...)` | Copy native ModelAssets result, native diag text, and summary. | KEEP_PRIMARY | Required physical-device artifact after pressing `Run True Engine ModelAssets Probe`; the current pass has `native_return=completed`, `persistent_custom_jni_status=completed`, `model_assets_create_reached=true`, `model_assets_create_returned=true`, `model_assets_create_succeeded=true`, and no EngineSettings/EngineFactory/Session/decode/generate reach. | `NpuTrueEngineHolderChatScreenStandardDebugBlockedTest` | Use before enabling `engine_settings_only`. |
...
- the Ten-Turn device result was not clean: `run_count_completed=7`,
