...
| `Run True Engine Holder Create/Close Probe` | `ChatScreen.kt`, `NpuTrueEngineHolderApi.kt`, `NpuTrueEngineHolderCreateCloseDevProbe.kt` | `startNpuTrueEngineHolderCreateCloseProbe()` -> startup-safe blocked result in `standardDebug`; `trueEngineNpuProbeDebug` reports staged marker payload with native execution disabled | Temporarily block the native true Engine create/close probe in `standardDebug` to restore app startup after the post-`63bfacfa` crash loop. | KEEP_PRIMARY | The current standard button path does not resolve model paths, load `Qairt244ShortMultitokenSmoke`, or call `litertlm_jni`. It reports `probe_execution_available=false`, `startup_native_call_blocked=true`, zero Session/decode/generate counts, and keeps `true_engine_persistent_reuse=false`. The isolated flavor plan is `docs/npu_true_engine_probe_flavor_isolation_plan.md`; the isolated APK may report `isolated_native_payload_staged=true` but must keep `isolated_native_execution_enabled=false` and `probe_execution_available=false`. | `NpuPersistentHolderNativeStubTest`, `NpuPersistentHolderApiTest`, `InferenceStatsSheetContentTest`, `NpuTrueEngineHolderChatScreenStandardDebugBlockedTest` | Verify isolated packaging and standard APK non-contamination, then add a separate execution gate only for `trueEngineNpuProbeDebug`. |
