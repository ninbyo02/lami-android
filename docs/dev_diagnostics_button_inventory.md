...
| `Run True Engine Holder Create/Close Probe` | `ChatScreen.kt`, `NpuTrueEngineHolderApi.kt`, `NpuTrueEngineHolderCreateCloseDevProbe.kt`, `Qairt244ShortMultitokenSmoke.kt` | `startNpuTrueEngineHolderCreateCloseProbe()` -> existing `litertlm_jni` persistent custom JNI with `nativeProbeMode=true_engine_create_close_only`, `runCount=0` | Exercise ModelAssets, EngineSettings, EngineFactory create, and Engine close without Session/decode/generate. | KEEP_PRIMARY | First true Engine reuse implementation step. The dedicated mode bypasses decode-mode run-count validation for create/close-only execution and checks native Engine create/close safety before split held-Engine run once. It does not prove reuse and keeps `true_engine_persistent_reuse=false`. | `NpuPersistentHolderNativeStubTest`, `NpuPersistentHolderApiTest`, `InferenceStatsSheetContentTest` | If device evidence passes, add a split held-Engine create/close API, then held-Engine run once with per-run Session. |
| `Copy True Engine Holder Summary` | `ChatScreen.kt`, `NpuTrueEngineHolderApi.kt` | `formatNpuTrueEngineHolderCreateCloseSummaryForCopy(...)` | Copy create/close-only native Engine safety keys. | KEEP_PRIMARY | Captures ModelAssets/EngineSettings/Engine create success, close success, zero Session/decode/generate counts, fatal latch, restart recommendation, and reuse non-claim fields. | `NpuPersistentHolderNativeStubTest`, `InferenceStatsSheetContentTest` | Keep as compact pass/hold artifact for held-Engine run-once readiness. |
| `Copy True Engine Holder Full Dump` | `ChatScreen.kt`, `NpuTrueEngineHolderApi.kt` | `formatNpuTrueEngineHolderCreateCloseFullDumpForCopy(...)` | Copy native return, native result file, native diag file, and summary for the true Engine create/close probe. | KEEP_PRIMARY | Required to see exactly where ModelAssets/EngineSettings/EngineFactory/close reached on physical device while confirming Session/decode/generate stayed at zero. | `NpuPersistentHolderNativeStubTest`, `InferenceStatsSheetContentTest` | Keep until a split true Engine holder native diagnostics API replaces the runCount=0 custom JNI wrapper. |
