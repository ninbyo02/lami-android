...
| `Run True Engine Holder Create/Close Probe` | `ChatScreen.kt`, `NpuTrueEngineHolderApi.kt`, `NpuTrueEngineHolderCreateCloseDevProbe.kt`, `Qairt244ShortMultitokenSmoke.kt` | `startNpuTrueEngineHolderCreateCloseProbe()` -> existing `litertlm_jni` persistent custom JNI with `runCount=0` | Exercise ModelAssets, EngineSettings, EngineFactory create, and Engine close without Session/decode/generate. | KEEP_PRIMARY | First true Engine reuse implementation step. It checks native Engine create/close safety before split held-Engine run once. It does not prove reuse and keeps `true_engine_persistent_reuse=false`. | `NpuPersistentHolderNativeStubTest`, `NpuPersistentHolderApiTest`, `InferenceStatsSheetContentTest` | If device evidence passes, add a split held-Engine create/close API, then held-Engine run once with per-run Session. |
