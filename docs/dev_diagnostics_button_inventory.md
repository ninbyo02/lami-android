...
| `Run True Engine Holder Create/Close Probe` | `ChatScreen.kt`, `NpuTrueEngineHolderApi.kt`, `NpuTrueEngineHolderCreateCloseDevProbe.kt` | `startNpuTrueEngineHolderCreateCloseProbe()` -> startup-safe blocked result | Temporarily block the native true Engine create/close probe to restore app startup after the post-`63bfacfa` crash loop. | KEEP_PRIMARY | The current button path does not resolve model paths, load `Qairt244ShortMultitokenSmoke`, or call `litertlm_jni`. It reports `probe_execution_available=false`, `startup_native_call_blocked=true`, zero Session/decode/generate counts, and keeps `true_engine_persistent_reuse=false`. | `NpuPersistentHolderNativeStubTest`, `NpuPersistentHolderApiTest`, `InferenceStatsSheetContentTest` | Rebuild and stage create/close-only native mode in isolation, then re-enable only after `standardDebug` startup is stable. |
