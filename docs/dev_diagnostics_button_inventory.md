...
| `Run True Engine Holder Create/Close Probe` | `ChatScreen.kt`, `NpuTrueEngineHolderApi.kt`, `NpuTrueEngineHolderCreateCloseDevProbe.kt` | `startNpuTrueEngineHolderCreateCloseProbe()` -> startup-safe blocked result in `standardDebug`; button-triggered create/close-only native path in `trueEngineNpuProbeDebug` | Temporarily block the native true Engine create/close probe in `standardDebug` to restore app startup after the post-`63bfacfa` crash loop while allowing isolated create/close-only execution in `trueEngineNpuProbeDebug`. | KEEP_PRIMARY | The standard button path does not resolve model paths, load `Qairt244ShortMultitokenSmoke`, or call `litertlm_jni`. It reports `probe_execution_available=false`, `startup_native_call_blocked=true`, zero Session/decode/generate counts, and keeps `true_engine_persistent_reuse=false`. The isolated APK may report `probe_execution_available=true`, but only the Run button may call `true_engine_create_close_only`; Session, decode, and generate must remain zero. | `NpuPersistentHolderNativeStubTest`, `NpuPersistentHolderApiTest`, `InferenceStatsSheetContentTest`, `NpuTrueEngineHolderChatScreenStandardDebugBlockedTest` | Review device create/close-only result. Proceed to held Engine run once only after ModelAssets, EngineSettings, EngineFactory, and close all succeed with zero Session/decode/generate. |
| `Copy True Engine Holder Summary` | `ChatScreen.kt`, `NpuTrueEngineHolderApi.kt` | `formatNpuTrueEngineHolderCreateCloseSummaryForCopy(...)` | Copy startup-safe blocked or future create/close-only Engine safety keys. | KEEP_PRIMARY | Initial and blocked summaries are pure Kotlin and include `true_engine_create_close_probe_startup_safe=true`, `native_call_deferred_until_button_click=true`, and `startup_native_call_blocked=true`. | `NpuPersistentHolderNativeStubTest`, `InferenceStatsSheetContentTest` | Keep as the compact artifact for startup recovery and future held-Engine run-once readiness. |
| `Copy True Engine Holder Full Dump` | `ChatScreen.kt`, `NpuTrueEngineHolderApi.kt` | `formatNpuTrueEngineHolderCreateCloseFullDumpForCopy(...)` | Copy blocked state or future native create/close details plus summary. | KEEP_PRIMARY | In the recovery build this does not include native result files because native execution is blocked. Future unblocking must still confirm Session/decode/generate stayed at zero. | `NpuPersistentHolderNativeStubTest`, `InferenceStatsSheetContentTest` | Keep until a split true Engine holder native diagnostics API replaces the runCount=0 custom JNI wrapper. |
