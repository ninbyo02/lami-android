...
failure twice while excluding pseudo streaming, TTS, DB writes, markdown
rendering, and normal chat UI side effects. Each check completed 6 successful
one-shot NPU decodes and failed on run 7, making
`first_failure_run_index=7` reproducible with
`adapter_failure:LiteRtLmJniException`; the native tail
...
all zero, and the summary keys now align on
`engine_create_failure_detected=true`, `suspected_failure_area=engine_create`,
...
This makes repeated one-shot recreate the leading suspect and
strengthens the case for true Engine reuse investigation, while still requiring
`trueEngineNpuProbeDebug` native execution to stay disabled until staged,
button-only probes are safe.
...
- Long Generation, R6 streaming, fallback policy, normal NPU chat route, and
  held Engine run once remain out of scope until staged true Engine
  probes pass through the entrypoint/ModelAssets/EngineSettings/before-create
  gates and a later create-only check.
- The `trueEngineNpuProbeDebug` shell now has isolated `jniLibs` / native
  ...
  staged Phase 2 button-only `entrypoint_only` is explicitly reopened.
...
true Engine reuse out of scope. The current device artifact has been
reproduced twice: both checks stopped at
`run_count_completed=7`, with `success_count=6`, `failure_count=1`,
`run_decode_reached_count=6`, `first_failure_run_index=7`,
`engine_create_failure_detected=true`, and
