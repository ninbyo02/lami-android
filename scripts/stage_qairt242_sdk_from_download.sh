...
INPUT="${1:-}"
DEST="${QAIRT242_DEST:-$HOME/compose/qairt/workspace/sdk/qairt/2.42.0.251225}"
EXPECTED_VERSION="2.42.0.251225"
KNOWN_BAD_VERSION="2.46.0.260424"

log() {
  printf '[stage-qairt242] %s\n' "$*"
}
...
Safely stages an officially obtained QAIRT 2.42.0.251225 SDK into:
  ~/compose/qairt/workspace/sdk/qairt/2.42.0.251225

The script refuses to overwrite an existing destination, rejects paths that
resolve to a known 2.46 SDK/overlay, and runs check_qairt242_sdk.sh after
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/qairt242_stage.XXXXXX")"
cleanup() {
  rm -rf "$WORK_DIR"
}
trap cleanup EXIT
...
case "$CANDIDATE_REALPATH" in
  *"$EXPECTED_VERSION"*) ;;
  *)
    if ! find "$CANDIDATE" -maxdepth 3 -type f 2>/dev/null | grep -E "$EXPECTED_VERSION|251225" >/dev/null; then
...
bash "$ROOT_DIR/scripts/check_qairt242_sdk.sh" "$DEST"
