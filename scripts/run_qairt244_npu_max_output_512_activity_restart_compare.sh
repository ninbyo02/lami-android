...
OUT_DIR="$ROOT_DIR/artifacts/qairt244_npu_max_output_512_activity_restart_compare/$TIMESTAMP"
...
NATIVE_BACKUP_DIR="/tmp/lami_qairt244_activity_restart_native_backup_$TIMESTAMP"
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --help|-h)
      cat <<'EOF'
Usage:
  scripts/run_qairt244_npu_max_output_512_activity_restart_compare.sh --artifact <native-build-artifact> [--device <serial>] [--timeout <seconds>] [--template <mode>]
  scripts/run_qairt244_npu_max_output_512_activity_restart_compare.sh --preflight-only --artifact <native-build-artifact>
...
The runner writes artifacts/qairt244_npu_max_output_512_activity_restart_compare/<timestamp>/.

Safety constraints:
  - max_output_tokens is capped at 512 by this runner.
  - exactly three approved prompts are executed once each.
  - Activity finish/restart is used between prompts.
  - app process force-stop is prohibited by this runner.
...
log() { printf '[qairt244-max-output-512-activity-restart] %s\n' "$*"; }

restore_native_source() {
  if [ "$RESTORE_NATIVE_SOURCE" != true ]; then
    return 0
  fi
  if [ -d "$NATIVE_BACKUP_DIR" ]; then
    cp -f "$NATIVE_BACKUP_DIR"/*.so "$STAGED_JNI_SOURCE_DIR"/ 2>/dev/null || true
...
activity_restart_step() {
  ...
  local run_dir="$2"
  {
    ...
    printf 'device=%s\n' "$DEVICE_SERIAL"
    printf 'process_force_stop_used=false\n'
    printf 'activity_finish_method=input_keyevent_back\n'
    adb_cmd shell pidof "$APP_ID" | sed 's/^/pid_before_finish=/' || true
    adb_cmd shell input keyevent KEYCODE_BACK
    sleep 2
    adb_cmd shell pidof "$APP_ID" | sed 's/^/pid_after_finish=/' || true
    printf 'activity_relaunch_method=am_start_main_activity\n'
    adb_cmd shell am start -W -n "$APP_ID/.MainActivity"
    adb_cmd shell pidof "$APP_ID" | sed 's/^/pid_after_relaunch=/' || true
    printf 'activity_restart_done=true\n'
  } >>"$OUT_DIR/activity_restart_sequence.md" 2>&1 || true
  cp "$OUT_DIR/activity_restart_sequence.md" "$run_dir/activity_restart_sequence_snapshot.md" 2>/dev/null || true
}
...
stage_build_and_install() {
  ...
  mkdir -p "$NATIVE_BACKUP_DIR" "$STAGED_JNI_SOURCE_DIR"
  cp -f "$STAGED_JNI_SOURCE_DIR"/*.so "$NATIVE_BACKUP_DIR"/ 2>/dev/null || true
...
run_prompt_512() {
...

  {
    ...
    printf 'execution_mode=activity_restart_only\n'
    printf 'process_force_stop_allowed=false\n'
    printf 'db=false\n'
    printf 'tts=false\n'
  ...
  if ! wait_for_state; then
    wait_status=timeout
    {
      printf 'timeout=true\n'
      printf 'process_force_stop_used=false\n'
      printf 'reason=timeout_but_force_stop_is_prohibited_for_activity_restart_only_test\n'
    } >"$run_dir/no_force_stop_timeout_note.txt"
  fi
  end_ms="$(date +%s000)"
  ...
  activity_restart_step "after_${index}_${slug}" "$run_dir"
  sleep 10
...

  {
    ...
    printf 'execution_mode=activity_restart_only\n'
    printf 'activity_restart_after_prompt=true\n'
    printf 'process_force_stop_used=false\n'
  ...
  qairt244_lifecycle_summary_lines "$run_dir" "$wait_status" "activity_restart_only" >>"$run_dir/case_summary.txt"
...
write_grep_safety() {
  {
    ...
    rg -n "allow_max_output_tokens_compare|QAIRT244_MAX_OUTPUT_TOKENS_COMPARE_LIMIT|max_output_tokens|selectedPath.*npu|selected_path_npu|tts=true|markdown=true|streaming=true|db=true|generateResponse|assistant_message_list|standard_route_connected|force-stop|force_stop" \
      scripts/run_qairt244_npu_max_output_512_activity_restart_compare.sh app/src/debug/java app/src/main/java app/src/customBuildExperimentDebug 2>&1 || true
    ...
    printf 'activity_restart_only=true\n'
    printf 'process_force_stop_used=false\n'
...
collect_max512_preflight_evidence() {
...

  {
    ...
    rg -n "choose_device|adb_cmd|RunDecode|am start|am broadcast|--preflight-only|collect_max512_preflight_evidence|require_max512_guard_evidence|db=false|tts=false|markdown=false|streaming=false|gemma-4-E2B-it_qualcomm_sm8750|SM8750|sm8750" \
      scripts/run_qairt244_npu_max_output_512_activity_restart_compare.sh scripts/check_qairt244_native_patch.sh docs 2>&1 || true
...
write_summary() {
  ...
  {
    printf '# QAIRT244 NPU max_output_tokens 512 activity-restart-only hidden comparison\n\n'
    printf -- '- artifact: `%s`\n' "${OUT_DIR#$ROOT_DIR/}"
    ...
    printf -- '- execution_isolation: `Activity finish/relaunch between prompts; process force-stop prohibited`\n'
    ...
    printf -- '- The runner uses Activity restart only and does not issue app process force-stop.\n'
...
write_three_prompt_notes() {
  cp "$OUT_DIR/runtime_marker_scan.txt" "$OUT_DIR/marker_scan.txt" 2>/dev/null || : >"$OUT_DIR/marker_scan.txt"
  {
    ...
    printf 'reuse_allowed_all=false\n'
    ...
    printf 'result_classification=%s\n' "$(awk -F= 'BEGIN { py_status="missing" } $1 == "prompt" { p=$2 } p == "Pythonで簡単な電卓コードを書いて" && $1 == "status" { py_status=$2 } END { if (py_status == "success") print "activity_restart_solves_timeout"; else if (py_status == "timeout") print "activity_restart_insufficient"; else print "unknown" }' "$OUT_DIR/case_summaries.txt")"
    printf 'next_phase=512_activity_restart_result_review_no_1024_yet\n'
...
main() {
  ...
  : >"$OUT_DIR/activity_restart_sequence.md"
