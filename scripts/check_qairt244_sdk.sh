...
OUT_DIR="${OUT_DIR:-$ROOT_DIR/artifacts/qairt244_sdk_check/$TIMESTAMP}"
QAIRT_ROOT="${1:-}"
EXPECTED_VERSION="2.44.0.260225"
KNOWN_BAD_VERSION="2.46.0.260424"

log() {
  printf '[check-qairt244] %s\n' "$*"
}
...
Checks that the given QAIRT root looks like the exact 2.44.0.260225 SDK.
This script does not build LiteRT, does not install an app, and does not
copy files into app source sets.
EOF
}
  ...
  local out="$2"
  {
    ...
    if [ -f "$file" ]; then
      ...
      strings "$file" 2>/dev/null |
        grep -Ei 'QNN|QAIRT|HTP|V79|2\.44|260225|version|build|sdk' |
        sort -u |
        head -200 || true
    fi
  } >"$out"
...
{
  printf '# QAIRT 2.44 SDK Check Summary\n\n'
  printf -- '- QAIRT root: `%s`\n' "$QAIRT_ROOT"
  printf -- '- Real path: `%s`\n' "$QAIRT_REALPATH"
