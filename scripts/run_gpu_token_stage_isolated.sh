...
# TOKENS is EngineConfig.maxNumTokens (input + output context), not generated output.
tokens="${1:?usage: $0 CONTEXT_TOKENS [RUNS] [TIMEOUT_MS]}"
runs="${2:-10}"
timeout_ms="${3:-60000}"
...
for run in $(seq 1 "${runs}"); do
  ...
  adb shell am broadcast --async -f 0x20 -a "${action}" \
    ...
    --es context_max_tokens_list "${tokens}" \
