...
OUT_DIR="$ROOT_DIR/artifacts/qairt244_npu_max_output_512_single_prompt/$TIMESTAMP"
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --help|-h)
      cat <<'EOF'
Usage:
  scripts/run_qairt244_npu_max_output_512_single_prompt.sh --artifact <native-build-artifact> [--device <serial>] [--timeout <seconds>] [--template <mode>]
  scripts/run_qairt244_npu_max_output_512_single_prompt.sh --preflight-only --artifact <native-build-artifact>

Runs the standardDebug hidden QAIRT244 SM8750 NPU route exactly once for the
fixed prompt こんにちは with sanitizer_only and max_output_tokens=512. The existing
...
The runner writes artifacts/qairt244_npu_max_output_512_single_prompt/<timestamp>/.

Safety constraints:
  - max_output_tokens is capped at 512 by this runner.
  - exactly one prompt is executed once.
...
if [ "$SINGLE_PROMPT" != "こんにちは" ]; then
  printf 'ERROR: this phase permits only prompt=こんにちは\n' >&2
  exit 2
fi
...
log() { printf '[qairt244-max-output-512-single] %s\n' "$*"; }
...
write_grep_safety() {
  {
    ...
    rg -n "allow_max_output_tokens_compare|QAIRT244_MAX_OUTPUT_TOKENS_COMPARE_LIMIT|max_output_tokens|selectedPath.*npu|selected_path_npu|tts=true|markdown=true|streaming=true|db=true|generateResponse|assistant_message_list|standard_route_connected" \
      scripts/run_qairt244_npu_max_output_512_single_prompt.sh app/src/debug/java app/src/main/java app/src/customBuildExperimentDebug 2>&1 || true
...
collect_max512_preflight_evidence() {
  ...
  {
    printf '# grep safety scan\n'
    printf 'script=%s\n' "scripts/run_qairt244_npu_max_output_512_single_prompt.sh"
    ...
    rg -n "choose_device|adb_cmd|RunDecode|am start|am broadcast|--preflight-only|collect_max512_preflight_evidence|require_max512_guard_evidence|db=false|tts=false|markdown=false|streaming=false|gemma-4-E2B-it_qualcomm_sm8750|SM8750|sm8750" \
      scripts/run_qairt244_npu_max_output_512_single_prompt.sh scripts/check_qairt244_native_patch.sh docs 2>&1 || true
...
write_summary() {
  local overall_status="$1"
  {
    if [ "$SINGLE_PROMPT_ONLY" = true ]; then
      printf '# QAIRT244 NPU max_output_tokens 512 single prompt verification\n\n'
    else
      printf '# QAIRT244 NPU max_output_tokens 512 quality/safety compare\n\n'
    fi
    printf -- '- artifact: `%s`\n' "${OUT_DIR#$ROOT_DIR/}"
...
write_single_prompt_aliases() {
  local slug="konnichiwa"
  cp "$OUT_DIR/result_512_${slug}.txt" "$OUT_DIR/result.txt" 2>/dev/null || : >"$OUT_DIR/result.txt"
  cp "$OUT_DIR/native_diag_512_${slug}.txt" "$OUT_DIR/native_diag.txt" 2>/dev/null || : >"$OUT_DIR/native_diag.txt"
  cp "$OUT_DIR/raw_output_512_${slug}.txt" "$OUT_DIR/raw_output.txt" 2>/dev/null || : >"$OUT_DIR/raw_output.txt"
  cp "$OUT_DIR/sanitized_output_512_${slug}.txt" "$OUT_DIR/sanitized_output.txt" 2>/dev/null || : >"$OUT_DIR/sanitized_output.txt"
  cp "$OUT_DIR/runtime_marker_scan.txt" "$OUT_DIR/marker_scan.txt" 2>/dev/null || : >"$OUT_DIR/marker_scan.txt"
  {
    printf '# QAIRT244 max_output_tokens=512 single prompt post-run notes\n\n'
    printf 'run_count=1\n'
    printf 'prompt=こんにちは\n'
    printf 'max_output_tokens=512\n'
    ...
    printf 'run_decode_reached=%s\n' "$(rg -q 'before RunDecode SetMaxOutputTokens\\(512\\)' "$OUT_DIR/native_diag.txt" && printf true || printf false)"
    printf 'quality_classification=%s\n' "$(summary_field quality_classification "こんにちは")"
    printf 'baseline_promotion=false\n'
    printf 'next_phase=512_three_prompt_compare_if_review_accepts\n'
