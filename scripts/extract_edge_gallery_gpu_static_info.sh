...
presence_from_file() {
  local file="$1"
  local needle="$2"
  if grep -Fiq "$needle" "$file" 2>/dev/null; then
    printf 'yes'
  else
    printf 'no'
  fi
...
edge_native_summary_for() {
  local lib="$1"
  awk -F '\t' -v lib="$lib" '
    $2 == "lib/arm64-v8a/" lib {
      printf "size=%s sha256=%s build_id=%s", $5, $6, ($7 == "" ? "unavailable" : $7)
      found=1
      exit
    }
    END {
...
lami_compare_summary_for() {
  local lib="$1"
  local compare_file="$OUTPUT_DIR/litert_gpu_accelerator_strings_edge_vs_lami.txt"
  if [ ! -f "$compare_file" ]; then
    printf 'unavailable; run native lib compare first'
    return
  fi
  awk -F '\t' -v lib="$lib" '
    $1 == "lami" && $2 == lib && $3 == "yes" {
      printf "size=%s sha256=%s", $4, $5
      found=1
      exit
    }
    END {
      if (!found) {
        printf "not_present_in_compare"
      }
    }
  ' "$compare_file"
}

write_runtime_alignment_summaries() {
  local summary_out="$OUTPUT_DIR/runtime_alignment_summary.md"
  local diff_out="$OUTPUT_DIR/runtime_stack_diff.md"
  local keyword_file="$ARTISAN_KEYWORD_PRESENCE"
  local lami_compare="$OUTPUT_DIR/litert_gpu_accelerator_strings_edge_vs_lami.txt"

  cat >"$summary_out" <<EOF
# Edge Gallery runtime alignment summary

Generated by \`scripts/extract_edge_gallery_gpu_static_info.sh\`.

## Executor names

- \`LlmGpuArtisanExecutor\`: $(presence_from_file "$ARTISAN_ALL_HITS" "LlmGpuArtisanExecutor")
- \`LlmLiteRtCompiledModelExecutor\`: $(presence_from_file "$ARTISAN_ALL_HITS" "LlmLiteRtCompiledModelExecutor")
- \`LlmLiteRtCompiledModelExecutorDynamic\`: $(presence_from_file "$ARTISAN_ALL_HITS" "LlmLiteRtCompiledModelExecutorDynamic")
- \`tflite_gpu_kv_cache\`: $(presence_from_file "$ARTISAN_ALL_HITS" "tflite_gpu_kv_cache")
- \`tflite_opencl_kv_cache\`: $(presence_from_file "$ARTISAN_ALL_HITS" "tflite_opencl_kv_cache")

## Backend / executor selection evidence

- Supported backend list includes artisan backends: $(presence_from_file "$ARTISAN_ALL_HITS" "Supported backends are: [CPU, GPU, NPU, GPU_ARTISAN, CPU_ARTISAN, GOOGLE_TENSOR_ARTISAN]")
- Runtime string for GPU to GPU_ARTISAN switch: $(presence_from_file "$ARTISAN_ALL_HITS" "Artisan model detected. Switching backend from GPU to GPU_ARTISAN.")
- Backend constraint match/mismatch strings: $(presence_from_file "$ARTISAN_ALL_HITS" "backend constraint mismatch. Model requires one of [")
- Preferred engine type strings: $(presence_from_file "$ALL_FOCUS" "Preferred engine types: [")

## GPU accelerator / KV cache evidence

- \`LiteRtRegisterGpuAccelerator\`: $(presence_from_file "$ALL_FOCUS" "LiteRtRegisterGpuAccelerator")
- \`LiteRtGpuEnvironmentCreate\`: $(presence_from_file "$ALL_FOCUS" "LiteRtGpuEnvironmentCreate")
- \`Statically linked GPU accelerator registered.\`: $(presence_from_file "$ALL_FOCUS" "Statically linked GPU accelerator registered.")
- \`tflite_gpu_kv_cache\`: $(presence_from_file "$ARTISAN_ALL_HITS" "tflite_gpu_kv_cache")
- \`tflite_opencl_kv_cache\`: $(presence_from_file "$ARTISAN_ALL_HITS" "tflite_opencl_kv_cache")

## Phase 5 interpretation

- Edge Gallery contains native evidence for both the public LiteRT compiled model executor and an artisan GPU executor path.
- The \`Artisan model detected\` string suggests executor selection can be runtime/model-metadata driven, not just Java \`Backend.GPU\`.
- Static extraction does not prove the observed Edge Gallery run selected \`GPU_ARTISAN\`; it proves that the route exists in the runtime stack.
- LAMI reflection currently sees public \`Backend\` candidates \`CPU,GPU,NPU\` only, so \`GPU_ARTISAN\` appears unavailable through the public API surface.
- The current LAMI Status Code 13 at \`llm_litert_compiled_model_executor.cc:735\` fits compiled model invoke failure in the public compiled-model executor path.

## Source files

- \`compiled_model_executor_context.txt\`
- \`gpu_artisan_executor_context.txt\`
- \`kv_cache_context.txt\`
- \`backend_constraint_context.txt\`
- \`backend_artisan_analysis/keyword_presence.tsv\`
EOF

  cat >"$diff_out" <<EOF
# Edge Gallery vs LAMI runtime stack diff

Generated by \`scripts/extract_edge_gallery_gpu_static_info.sh\`.

| Category | Edge Gallery | LAMI | Difference | Risk |
| --- | --- | --- | --- | --- |
| libLiteRt.so | $(edge_native_summary_for "libLiteRt.so") | $(lami_compare_summary_for "libLiteRt.so") | SHA/build ID differs when compare data is present. | High: LiteRT GPU accelerator and compiled model ABI must stay aligned. |
| liblitertlm_jni.so | $(edge_native_summary_for "liblitertlm_jni.so") | $(lami_compare_summary_for "liblitertlm_jni.so") | SHA/build ID/size differs when compare data is present. | High: executor selection and JNI/native API behavior live here. |
| Build IDs | See \`native_lib_inventory.tsv\`. | See native compare output if available. | Not guaranteed aligned. | High for mixed-stack experiments. |
| Backend APIs | Static strings include CPU/GPU/NPU and artisan backend names. | Runtime reflection reports CPU/GPU/NPU public candidates only. | Artisan route is visible statically in Gallery but not public in LAMI. | High for Backend.GPU vs GPU_ARTISAN mismatch. |
| Executor hints | \`LlmGpuArtisanExecutor\`, \`LlmLiteRtCompiledModelExecutor\`, dynamic CPU-only executor string present. | Public reflection has no GPU_ARTISAN executor surface. | Executor selection may differ. | High. |
| GPU accelerator registration | \`LiteRtRegisterGpuAccelerator\`, \`LiteRtGpuEnvironmentCreate\`, static GPU registration strings present. | Previous LAMI string compare shows GPU accelerator strings also present. | GPU feature is not simply missing; behavior differs deeper. | Medium. |
| KV cache hints | \`tflite_gpu_kv_cache\`, \`tflite_opencl_kv_cache\`, prefill/decode strings present. | LAMI comparable strings should be checked with native string compare. | KV cache path may differ by runtime/executor. | Medium-high. |
| Model constraints | Backend constraint match/mismatch and preferred engine type strings present. | Public API has no backend constraint setter/getter detected so far. | Constraint handling may be internal/runtime-driven. | High. |
| RuntimeConfig | Native strings include RuntimeConfig hints. | Public reflection currently reports RuntimeConfig availability separately in DEV diag. | RuntimeConfig may be native/internal rather than usable public API. | Medium-high. |
| EngineConfig | Java constructor string is visible in dex strings. | LAMI uses public EngineConfig backend fields. | Same Java shape does not imply same native executor selection. | Medium. |

## Notes

- \`libLiteRt.so\` and \`liblitertlm_jni.so\` are set-dependent. Do not replace one without a fully isolated flavor/runtime stack.
- Edge Gallery runtime must not be copied into LAMI from APK artifacts.
- \`libQnnGpu.so\` is not the primary GPU hypothesis because Edge Gallery GPU evidence is in LiteRT/LiteRT-LM GPU stack strings, not QNN GPU.

## Compare artifact availability

- LAMI string compare file present: $([ -f "$lami_compare" ] && printf 'yes' || printf 'no')
- Edge keyword presence file: \`$keyword_file\`
EOF
}
...
if [ "$DRY_RUN" = "1" ]; then
  printf 'dry_run=true\n'
  printf '%s\n' "$APK_LIST"
  printf 'planned_outputs=summary.txt,apk_inventory.tsv,native_lib_inventory.tsv,apk_entries/,native_libs/,strings/,backend_artisan_analysis/,compiled_model_executor_context.txt,gpu_artisan_executor_context.txt,kv_cache_context.txt,backend_constraint_context.txt,runtime_alignment_summary.md,runtime_stack_diff.md,app_data_static_check_instructions.md\n'
  exit 0
fi
...

{
  ...
  printf 'runtime_alignment_summary=%s\n' "$OUTPUT_DIR/runtime_alignment_summary.md"
  printf 'runtime_stack_diff=%s\n' "$OUTPUT_DIR/runtime_stack_diff.md"
