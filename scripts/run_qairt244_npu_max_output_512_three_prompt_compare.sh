...
OUT_DIR="$ROOT_DIR/artifacts/qairt244_npu_max_output_512_three_prompt_compare/$TIMESTAMP"
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --help|-h)
      cat <<'EOF'
Usage:
  scripts/run_qairt244_npu_max_output_512_three_prompt_compare.sh --artifact <native-build-artifact> [--device <serial>] [--timeout <seconds>] [--template <mode>]
  scripts/run_qairt244_npu_max_output_512_three_prompt_compare.sh --preflight-only --artifact <native-build-artifact>
...
The runner writes artifacts/qairt244_npu_max_output_512_three_prompt_compare/<timestamp>/.
...
log() { printf '[qairt244-max-output-512-three] %s\n' "$*"; }
...
write_grep_safety() {
  {
    ...
    rg -n "allow_max_output_tokens_compare|QAIRT244_MAX_OUTPUT_TOKENS_COMPARE_LIMIT|max_output_tokens|selectedPath.*npu|selected_path_npu|tts=true|markdown=true|streaming=true|db=true|generateResponse|assistant_message_list|standard_route_connected" \
      scripts/run_qairt244_npu_max_output_512_three_prompt_compare.sh app/src/debug/java app/src/main/java app/src/customBuildExperimentDebug 2>&1 || true
...
collect_max512_preflight_evidence() {
  ...
  {
    printf '# grep safety scan\n'
    printf 'script=%s\n' "scripts/run_qairt244_npu_max_output_512_three_prompt_compare.sh"
    ...
    rg -n "choose_device|adb_cmd|RunDecode|am start|am broadcast|--preflight-only|collect_max512_preflight_evidence|require_max512_guard_evidence|db=false|tts=false|markdown=false|streaming=false|gemma-4-E2B-it_qualcomm_sm8750|SM8750|sm8750" \
      scripts/run_qairt244_npu_max_output_512_three_prompt_compare.sh scripts/check_qairt244_native_patch.sh docs 2>&1 || true
...
write_summary() {
  ...
  {
    printf '# QAIRT244 NPU max_output_tokens 512 three-prompt hidden comparison\n\n'
    printf -- '- artifact: `%s`\n' "${OUT_DIR#$ROOT_DIR/}"
...
write_three_prompt_notes() {
  cp "$OUT_DIR/runtime_marker_scan.txt" "$OUT_DIR/marker_scan.txt" 2>/dev/null || : >"$OUT_DIR/marker_scan.txt"
  {
    ...
    printf 'hidden_baseline_candidate=%s\n' "$(awk -F= '$1 == "status" && $2 != "success" { bad=1 } $1 == "quality_classification" && $2 != "natural_japanese" && $2 != "useful_code" && $2 != "useful_long_response" { bad=1 } END { print bad ? "false" : "true" }' "$OUT_DIR/case_summaries.txt")"
    printf 'next_phase=1024_guard_patch_preflight_if_review_accepts\n'
