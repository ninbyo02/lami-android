...
RUNTIME_ALIGNMENT_APK="$ROOT_DIR/app/build/outputs/apk/gpuRuntimeAlignmentProbe/debug/app-gpuRuntimeAlignmentProbe-debug.apk"
OUT_DIR="$ROOT_DIR/artifacts/gpu_runtime_stack_compare"

usage() {
  cat <<USAGE
usage: $0 [--standard-apk PATH] [--runtime-alignment-apk PATH] [--out-dir PATH]

Compares final APK arm64-v8a native libraries between standardDebug and
gpuRuntimeAlignmentProbeDebug. Outputs TSV/Markdown diagnostics only; it never
stages, copies, or modifies native libraries.

defaults:
  --standard-apk          $STANDARD_APK
  --runtime-alignment-apk $RUNTIME_ALIGNMENT_APK
  --out-dir               $OUT_DIR
USAGE
}
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    --standard-apk)
      ...
      shift 2
      ;;
    --runtime-alignment-apk|--probe-apk)
      RUNTIME_ALIGNMENT_APK="${2:-}"
      shift 2
      ;;
    --out-dir)
      OUT_DIR="${2:-}"
...
if [ ! -f "$RUNTIME_ALIGNMENT_APK" ]; then
  printf 'missing gpuRuntimeAlignmentProbeDebug APK: %s\n' "$RUNTIME_ALIGNMENT_APK" >&2
  printf 'hint: run ./gradlew :app:assembleGpuRuntimeAlignmentProbeDebug first\n' >&2
  exit 1
fi
...
RUNTIME_ALIGNMENT_EXTRACT="$WORK_DIR/gpu-runtime-alignment"
extract_apk_libs "$STANDARD_APK" "standardDebug" "$STANDARD_EXTRACT" || exit 1
extract_apk_libs "$RUNTIME_ALIGNMENT_APK" "gpuRuntimeAlignmentProbeDebug" "$RUNTIME_ALIGNMENT_EXTRACT" || exit 1
STANDARD_DIR="$STANDARD_EXTRACT/lib/arm64-v8a"
RUNTIME_ALIGNMENT_DIR="$RUNTIME_ALIGNMENT_EXTRACT/lib/arm64-v8a"

STANDARD_INVENTORY="$OUT_DIR/standard_debug_native_libs.tsv"
RUNTIME_ALIGNMENT_INVENTORY="$OUT_DIR/gpu_runtime_alignment_probe_native_libs.tsv"
...
write_needed_edges "gpuRuntimeAlignmentProbeDebug" "$RUNTIME_ALIGNMENT_DIR" "$NEEDED_OUT"

{
  find "$STANDARD_DIR" "$RUNTIME_ALIGNMENT_DIR" -maxdepth 1 -type f -name '*.so' 2>/dev/null |
    sed 's#.*/##' |
    sort -u
} >"$OUT_DIR/all_native_lib_names.txt"

printf 'library\tstandard_present\truntime_alignment_present\tsame_sha256\tsame_build_id\tstandard_size_bytes\truntime_alignment_size_bytes\tstandard_sha256\truntime_alignment_sha256\tstandard_build_id\truntime_alignment_build_id\tstandard_needed\truntime_alignment_needed\tcategory\tclassification\tnotes\n' >"$DIFF_OUT"

while IFS= read -r lib; do
  [ -n "$lib" ] || continue
  standard_file="$STANDARD_DIR/$lib"
  runtime_alignment_file="$RUNTIME_ALIGNMENT_DIR/$lib"
  standard_present="no"
  runtime_alignment_present="no"
  [ -f "$standard_file" ] && standard_present="yes"
  [ -f "$runtime_alignment_file" ] && runtime_alignment_present="yes"
  standard_size=""
  runtime_alignment_size=""
  standard_sha=""
  runtime_alignment_sha=""
  standard_build=""
  runtime_alignment_build=""
  standard_needed=""
  runtime_alignment_needed=""
  ...
  if [ "$runtime_alignment_present" = "yes" ]; then
    runtime_alignment_size="$(size_for "$runtime_alignment_file")"
    runtime_alignment_sha="$(sha_for "$runtime_alignment_file")"
    runtime_alignment_build="$(build_id_for "$runtime_alignment_file")"
    runtime_alignment_needed="$(needed_for "$runtime_alignment_file")"
  fi
  same_sha="false"
  same_build="false"
  if [ "$standard_present" = "yes" ] && [ "$runtime_alignment_present" = "yes" ] && [ -n "$standard_sha" ] && [ "$standard_sha" = "$runtime_alignment_sha" ]; then
    same_sha="true"
  fi
  if [ "$standard_present" = "yes" ] && [ "$runtime_alignment_present" = "yes" ] && [ -n "$standard_build" ] && [ "$standard_build" = "$runtime_alignment_build" ]; then
    same_build="true"
  fi
  category="$(category_for "$lib")"
  classification="$(classification_for "$lib" "$standard_present" "$runtime_alignment_present" "$same_sha" "$same_build")"
  notes="$(notes_for "$lib" "$classification")"
  printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
    "$lib" \
    "$standard_present" \
    "$runtime_alignment_present" \
    "$same_sha" \
    "$same_build" \
    "$(field_or_none "$standard_size")" \
    "$(field_or_none "$runtime_alignment_size")" \
    "$(field_or_none "$standard_sha")" \
    "$(field_or_none "$runtime_alignment_sha")" \
    "$(field_or_none "$standard_build")" \
    "$(field_or_none "$runtime_alignment_build")" \
    "$(field_or_none "$standard_needed")" \
    "$(field_or_none "$runtime_alignment_needed")" \
    "$category" \
    "$classification" \
    "$notes"
...

{
  ...
  printf '%s\n' "- gpu_runtime_alignment_probe_apk: \`$RUNTIME_ALIGNMENT_APK\`"
  ...
  printf 'The current model-separation probe showed the same Edge Gallery E2B model succeeds in `gpuRuntimeAlignmentProbe` and fails in `standardDebug`. Treat remaining deltas as runtime/native stack candidates, not model identity candidates.\n\n'
  ...
  printf '| Library | Category | Classification | standardDebug | gpuRuntimeAlignmentProbeDebug | Notes |\n'
  printf '| --- | --- | --- | --- | --- | --- |\n'
  for focus in \
    libLiteRt.so \
    liblitertlm_jni.so \
    ...
    awk -F '\t' -v lib="$focus" '
      NR > 1 && $1 == lib {
        ...
        probe = $3 ", size=" $7 ", build_id=" $11
        printf("| `%s` | `%s` | `%s` | `%s` | `%s` | %s |\n", $1, $14, $15, std, probe, $16)
      }
    ' "$DIFF_OUT"
  done
  printf '\n## Outputs\n\n'
