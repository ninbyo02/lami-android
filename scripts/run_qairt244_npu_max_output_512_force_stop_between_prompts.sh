...
OUT_DIR="$ROOT_DIR/artifacts/qairt244_npu_max_output_512_force_stop_between_prompts/$TIMESTAMP"
...
NATIVE_BACKUP_DIR="/tmp/lami_qairt244_force_stop_native_backup_$TIMESTAMP"
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --help|-h)
      cat <<'EOF'
Usage:
  scripts/run_qairt244_npu_max_output_512_force_stop_between_prompts.sh --artifact <native-build-artifact> [--device <serial>] [--timeout <seconds>] [--template <mode>]
  scripts/run_qairt244_npu_max_output_512_force_stop_between_prompts.sh --preflight-only --artifact <native-build-artifact>
...
The runner writes artifacts/qairt244_npu_max_output_512_force_stop_between_prompts/<timestamp>/.
...
run_prompt_512() {
  ...
  force_stop_app "before_${index}_${slug}" "$OUT_DIR/force_stop_sequence.md"
  sleep 2
  ...
  force_stop_app "after_${index}_${slug}" "$OUT_DIR/force_stop_sequence.md"
  sleep 10
...
write_comparison_table() {
  ...
  {
    printf '# QAIRT244 NPU max_output_tokens 512 force-stop quality/safety comparison\n\n'
    printf '256 hidden experimental candidate reference: `%s`\n\n' "${BASELINE_DIR#$ROOT_DIR/}"
    printf 'Execution isolation: force-stop before and after each prompt; one run per approved prompt.\n\n'
...
collect_max512_preflight_evidence() {
...

  {
    ...
    rg -n "choose_device|adb_cmd|RunDecode|am start|am broadcast|--preflight-only|collect_max512_preflight_evidence|require_max512_guard_evidence|db=false|tts=false|markdown=false|streaming=false|gemma-4-E2B-it_qualcomm_sm8750|SM8750|sm8750" \
      scripts/run_qairt244_npu_max_output_512_force_stop_between_prompts.sh scripts/check_qairt244_native_patch.sh docs 2>&1 || true
...
write_summary() {
  ...
  {
    printf '# QAIRT244 NPU max_output_tokens 512 force-stop between prompts comparison\n\n'
    printf -- '- artifact: `%s`\n' "${OUT_DIR#$ROOT_DIR/}"
    ...
    printf -- '- execution_isolation: `force-stop before and after each prompt`\n'
...
main() {
  ...
  : >"$OUT_DIR/force_stop_sequence.md"
