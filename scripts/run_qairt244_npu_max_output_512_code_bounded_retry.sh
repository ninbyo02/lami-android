...
OUT_DIR="$ROOT_DIR/artifacts/qairt244_npu_max_output_512_code_bounded_retry/$TIMESTAMP"
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --help|-h)
      cat <<'EOF'
Usage:
  scripts/run_qairt244_npu_max_output_512_code_bounded_retry.sh --artifact <native-build-artifact> [--device <serial>] [--timeout 60] [--template <mode>]
  scripts/run_qairt244_npu_max_output_512_code_bounded_retry.sh --preflight-only --artifact <native-build-artifact>

Runs the standardDebug hidden QAIRT244 SM8750 NPU route once for the approved
Python calculator prompt with sanitizer_only, max_output_tokens=512, and a
bounded timeout of at most 60 seconds. The existing
artifacts/qairt244_npu_max_output_256_three_prompt_compare/20260526_211856
artifact is used as the 256 hidden experimental candidate reference; this
runner does not rerun 256.
...
The runner writes artifacts/qairt244_npu_max_output_512_code_bounded_retry/<timestamp>/.

Safety constraints:
  - max_output_tokens is capped at 512 by this runner.
  - exactly one approved Python prompt is executed once.
  - timeout must be bounded and must not exceed 60 seconds.
...
if [ "$TIMEOUT_SECONDS" -gt 60 ]; then
  printf 'ERROR: --timeout must be <=60 for this bounded retry\n' >&2
  exit 2
fi
...
log() { printf '[qairt244-max-output-512-code-retry] %s\n' "$*"; }
...
capture_screenshot() {
  local slug="$1"
  local run_dir="$2"
  local remote_png="/sdcard/qairt244_max_output_512_${slug}.png"
  ...
  cp "$run_dir/screenshot.png" "$OUT_DIR/screenshot_512_${slug}.png" 2>/dev/null || true
}
...
write_comparison_table() {
  local prompt status quality decode elapsed len npu fallback fresh timeout selected baseline_quality baseline_decode baseline_status
  {
    ...
    printf '| prompt | 256_ref_status | 256_ref_quality | 256_ref_decode_ms | 512_status | 512_quality | 512_decode_ms | 512_elapsed_ms | 512_len | npu_evidence | fallback_used | timeout | fresh_crash | selected_path_npu_saved |\n'
    printf '| --- | --- | --- | ---: | --- | --- | ---: | ---: | ---: | --- | --- | --- | --- | --- |\n'
...
write_grep_safety() {
  {
    ...
    rg -n "allow_max_output_tokens_compare|QAIRT244_MAX_OUTPUT_TOKENS_COMPARE_LIMIT|max_output_tokens|selectedPath.*npu|selected_path_npu|tts=true|markdown=true|streaming=true|db=true|generateResponse|assistant_message_list|standard_route_connected" \
      scripts/run_qairt244_npu_max_output_512_code_bounded_retry.sh app/src/debug/java app/src/main/java app/src/customBuildExperimentDebug 2>&1 || true
...
collect_max512_preflight_evidence() {
...

  {
    ...
    rg -n "choose_device|adb_cmd|RunDecode|am start|am broadcast|--preflight-only|collect_max512_preflight_evidence|require_max512_guard_evidence|db=false|tts=false|markdown=false|streaming=false|gemma-4-E2B-it_qualcomm_sm8750|SM8750|sm8750" \
      scripts/run_qairt244_npu_max_output_512_code_bounded_retry.sh scripts/check_qairt244_native_patch.sh docs 2>&1 || true
...
write_summary() {
  ...
  {
    printf '# QAIRT244 NPU max_output_tokens 512 code prompt bounded retry\n\n'
    printf -- '- artifact: `%s`\n' "${OUT_DIR#$ROOT_DIR/}"
    ...
    printf -- '- run_count_policy: `single approved Python prompt, one run only`\n'
...
write_retry_artifacts() {
  cp "$OUT_DIR/result_1.txt" "$OUT_DIR/result.txt" 2>/dev/null || : >"$OUT_DIR/result.txt"
  cp "$OUT_DIR/native_diag_1.txt" "$OUT_DIR/native_diag.txt" 2>/dev/null || : >"$OUT_DIR/native_diag.txt"
  cp "$OUT_DIR/raw_output_1.txt" "$OUT_DIR/raw_output.txt" 2>/dev/null || : >"$OUT_DIR/raw_output.txt"
  cp "$OUT_DIR/sanitized_output_1.txt" "$OUT_DIR/sanitized_output.txt" 2>/dev/null || : >"$OUT_DIR/sanitized_output.txt"
  cp "$OUT_DIR/runtime_marker_scan.txt" "$OUT_DIR/marker_scan.txt" 2>/dev/null || : >"$OUT_DIR/marker_scan.txt"
  {
    printf 'timeout_seconds=%s\n' "$TIMEOUT_SECONDS"
    printf 'timeout_bounded=true\n'
    printf 'timeout_max_allowed_seconds=60\n'
    printf 'timeout_unlimited=false\n'
    printf 'prompt=Pythonで簡単な電卓コードを書いて\n'
    printf 'max_output_tokens=512\n'
  } >"$OUT_DIR/timeout_config.txt"
  if [ "$(summary_field status "Pythonで簡単な電卓コードを書いて")" = timeout ]; then
    {
      printf '# force stop note\n\n'
      printf 'force_stop_issued=true\n'
      printf 'reason=runner_timeout\n'
      printf 'timeout_seconds=%s\n' "$TIMEOUT_SECONDS"
      printf 'source=run_512_python_calculator/force_stop_timeout.txt\n'
    } >"$OUT_DIR/force_stop_note.txt"
  fi
  {
    local status quality timeout fresh fallback classification
    status="$(summary_field status "Pythonで簡単な電卓コードを書いて")"
    quality="$(summary_field quality_classification "Pythonで簡単な電卓コードを書いて")"
    timeout="$(summary_field timeout "Pythonで簡単な電卓コードを書いて")"
    fresh="$(summary_field fresh_crash "Pythonで簡単な電卓コードを書いて")"
    fallback="$(summary_field fallback_used "Pythonで簡単な電卓コードを書いて")"
    classification=other
    if [ "$status" = success ] && [ "$quality" = useful_code ]; then
      classification=success_but_slow
    elif [ "$status" = timeout ]; then
      if [ -s "$OUT_DIR/raw_output.txt" ] || [ -s "$OUT_DIR/sanitized_output.txt" ]; then
        classification=timeout_60s_with_partial_output
      else
        classification=timeout_60s_native_no_callback
      fi
    elif [ "$fresh" = true ]; then
      classification=crash_or_process_death
    fi
    printf '# QAIRT244 max_output_tokens=512 code bounded retry post-run notes\n\n'
    printf 'run_count=1\n'
    printf 'prompt=Pythonで簡単な電卓コードを書いて\n'
    printf 'max_output_tokens=512\n'
    printf 'timeout_seconds=%s\n' "$TIMEOUT_SECONDS"
    ...
    printf 'run_decode_reached=%s\n' "$(rg -q 'before RunDecode SetMaxOutputTokens\(512\)' "$OUT_DIR/native_diag.txt" 2>/dev/null && printf true || printf false)"
    printf 'cleanup_engine_close_evidence=%s\n' "$(rg -q 'Engine.close|cleanup_elapsed_ms' "$OUT_DIR/native_diag.txt" 2>/dev/null && printf true || printf false)"
    printf 'status=%s\n' "$status"
    printf 'quality_classification=%s\n' "$quality"
    printf 'timeout=%s\n' "$timeout"
    printf 'fresh_crash=%s\n' "$fresh"
    ...
    printf 'hidden_baseline_candidate=false\n'
    printf 'next_1024=false\n'
...
main() {
  ...
  if ! rg -q 'Engine.close|cleanup_elapsed_ms' "$OUT_DIR/native_diag.txt" 2>/dev/null; then
    overall_status=failure
  fi
